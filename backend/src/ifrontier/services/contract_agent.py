    ...
    def _draft_with_llm(
        ...
        *,
        ...
        system = (
            ...
            "       - condition: JSON 布尔表达式（标准语法，仅允许 op/args 或 op/left/right 形式；也允许直接 true/false）。",
            "         允许：{\"op\":\"and\",\"args\":[...]}/{\"op\":\"or\",\"args\":[...]}/{\"op\":\"not\",\"arg\":...}/{\"op\":\"==\",\"left\":...,\"right\":...} 等。",
            "         禁止：{\"==\":[...]}、{\"and\":[...]} 这类简写（会导致执行失败）。",
            "         变量只允许：{\"var\":\"cash:<account_id>\"} / {\"var\":\"pos:<account_id>:<symbol>\"} / {\"var\":\"price:<symbol>\"} / {\"var\":\"contract.status\"} / {\"var\":\"contract.runs:<rule_id>\"}",
            ...
            "   3.4 quantity 可为数字或 {\"expr\":<expr>}；expr 支持 op(add/sub/mul/div/min/max)、args、变量引用必须写成 {\"var\":\"cash:<account_id>\"}/{\"var\":\"pos:<account_id>:<symbol>\"}/{\"var\":\"price:<symbol>\"}/{\"var\":\"contract.runs:<contract_id>:<rule_id>\"}。",
            "       禁止写成 {\"op\":\"var\",\"args\":...}，也不要把变量包在 expr.var 之外的自定义结构里。",
            "       正确示例：{\"quantity\":{\"expr\":{\"var\":\"cash:user:chen\"}}} / {\"quantity\":{\"expr\":{\"op\":\"mul\",\"args\":[{\"var\":\"price:BLUEGOLD\"},2]}}}",
            ...
            "7. 执行时机约束（强制遵守）：规则只会在合约进入 ACTIVE 后被调度执行。若希望签署完成立刻执行一次性转账，请使用 schedule.type='once' 且 condition=true；不要写 condition=contract.status=='SIGNED'。",
            "8. 账户与资产规范（强制遵守）：所有 user_id/account_id 一律小写（例如 user:chen）；现金必须 asset_type='CASH' 且 symbol='CASH'；股票/权益必须 asset_type='EQUITY' 且 symbol 为证券代码（如 BLUEGOLD）。",
