...
补充原则（v0.1）：原版本与篡改版本并行传播
- 当 A 传播了一条新闻版本 `vA`，B 基于 `vA` 篡改得到 `vB` 后：
  - `vA` 与 `vB` 都会继续在社交网络中独立传播。
  - 同一个玩家可能先后收到 `vA` 与 `vB`（甚至在较近的游戏时间窗口内同时出现）。
  - 这种“真假/多版本并存”是系统刻意保留的机制：让市场在信息冲突中做决断。
- 只有当某方投入现金进行压制（Suppress）时，才会降低/阻断某条版本或某条事件链的投递覆盖。
...
补充（v0.1）：Suppress 的语义边界
- 压制应明确作用范围：
  - `scope=chain`：对某条事件链的前兆/孵化投递做预算压制（当前已实现）。
  - `scope=variant`（后续扩展）：针对某条具体版本的传播预算/覆盖压制。
- 若未被压制：允许同一玩家同时持有多个版本（vA/vB/…）的投递记录。
...
补充（v0.1）：新闻的时效性使用游戏内时间
- 新闻的“时效性”（例如：过期、衰减、传播窗口、T=0 落地）统一以**游戏内时间**为准，而不是现实时间。
- 后端以 `GameTime`（`IF_GAME_EPOCH_UTC`/`IF_SECONDS_PER_GAME_DAY`/`IF_GAME_NOW_UTC`）作为唯一权威时间源。
- 对外接口可继续接受 ISO 时间（用于测试/调试），但其语义是“游戏内时间的当前刻”。
